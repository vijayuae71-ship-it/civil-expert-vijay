...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Only an explicit boolean true enables the alternate format; absent or malformed values keep normal mode.
    const useQuoteReadyFormat = quoteReady === true;
...
${useQuoteReadyFormat ? `QUOTE-READY FORMAT IS ENABLED. This format overrides the normal answer framework, word limit, recommendation/reasoning headings, and generic next-step requirement. Produce only a compact quotation-ready output in this exact order: **Project / Scope Title**; **Assumptions**; an itemised rendered Markdown **BOQ** table where applicable; **Subtotals / Grand Total**; **Rate Basis**; **Exclusions**; and **Validity / Next Action**. Omit extended explanation, calculation or reasoning narrative, alternatives, common mistakes, generic introductions, and all other sections. Keep assumptions, exclusions, and validity/next-action to essential commercial bullets. If quantities or rates are unavailable, retain the relevant BOQ row and clearly mark it “To be verified”; do not invent figures. The CPWD/market-rate safety rules still apply without exception: preliminary market or project rates must be labelled as such and must never be represented as verified CPWD DSR rates.` : ""}
