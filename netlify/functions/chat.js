...
exports.handler = async (event) => {
  ...
  try {
    ...
    const quoteModeInstruction = quoteReady === true
      ? "\n\nQUOTE-READY MODE IS SELECTED. This instruction overrides the normal answer framework for construction materials, BOQs, cost, quote, budget, estimate, and rate-analysis requests. Return only this complete compact quotation, in this exact order: **Scope**, **Assumptions**, one complete itemised Markdown BOQ table, **Grand Total** (or a clear range), **Rate basis**, **Exclusions**, and **Validity / Next Step**. Do not add Recommendation, Calculation / Reasoning, teaching, alternatives, questions, or background. The BOQ table is mandatory and must contain both its header and every priced item before the reply ends. Use no more than 5 priced rows and no more than 350 words. If an exact CPWD entry/rate is not page-verified, write “To be verified against applicable CPWD DSR” in the CPWD reference and DSR Rate cells, and use a separately labelled “Preliminary market estimate” only where a first-pass price is useful. Never label a preliminary market rate as CPWD. Do not end immediately after a table header, and do not leave any heading, row, subtotal, or sentence unfinished."
      : "";
